function [blurImg,cornerImg,corners,filterCorner,Ix,Iy ] = myHarrisCornerDetector(img,windowSize,blurSigma,sigma,k)
%   Corner detection
    [row,col]=size(img);
    %
    ...
    for i=4:row-4
        ...
        for j=4:col-4
            ...
            val=harrisCornnerDetector(paddedIx(x1:x2,y1:y2),paddedIy(x1:x2,y1:y2),W,k);
            if val>minThreshold
                out(i,j)=val;
                corners=[corners;j,i];
